...
def fig1():
    ...
    g = iv.OVERCONFIDENT.gamma
    thr_c, thr_o = up_c[1], up_o[1] / g
    lp = g * L
    ...
    ax.text(xr, thr_o + 0.15, f"the overconfident person acts above this line\n(each observation counted {int(g)} times over)", ha="right", va="bottom", fontsize=7.5, color=INK2)
    ...
    ax.annotate("step %d: the overconfident person acts.\nThe action is final, and the state is bad." % t_o,
                xy=(t_o + 0.12, L[t_o - 1] - 0.15), xytext=(t_o + 1.6, thr_o - 1.4), va="top", ha="left", fontsize=7.5, color=INK2,
...
def fig2():
    rows = read("fig2.csv")
    N = rows[0]["n"]
    gammas = [1.0, 2.0, 4.0, 6.0]; ks = sorted({int(r["k"]) for r in rows})
    ...
    fig.legend([h for h, _ in handles] + [hf], [n for _, n in handles] + [f"failures after acting in the good state, γ = {iv.GAMMA_OVER:g} (panel a)"],
...
def fig3():
    ...
    for ax, person, letter, title in zip(axes, ("anxious", "fearful"), "ab",
        ...
        w9 = pick(person, f"verdict theta={iv.THETA}", "wait"); g9 = pick(person, f"verdict theta={iv.THETA}", "go")
        cells = [none, a0, w9, g9]
        ...
        for x, r, c in zip(xs, cells, colors):
            ...
            ax.text(x, 100 * v + 100 * ci(v, n) + 0.02 * max(100 * rr["missed"] for rr in cells), f"{100*v:.1f}%", ha="center", va="bottom", fontsize=7.5, color=INK)
        hcal = ax.axhline(100 * cal["missed"], color=INK2, lw=1, ls="--")
        ax.set_xticks(xs)
        ax.set_xticklabels(["no\ndevice", "random verdict,\nno relief\n(θ = 0)", "drew\n\"wait\"", "drew\n\"go\""], fontsize=7.5)
        ymax = max(100 * r["missed"] for r in cells) * 1.2
        ax.plot([1.72, 2.88], [-0.36 * ymax, -0.36 * ymax], color=INK2, lw=0.8, clip_on=False)
        ax.text(2.3, -0.42 * ymax, f"random verdict with relief (θ = {iv.THETA:g})", ha="center", va="top", fontsize=7.5, color=INK, clip_on=False)
        ax.set_xlim(-0.55, 3.6); ax.set_ylim(0, ymax)
