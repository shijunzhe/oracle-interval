...
| `results/` | One CSV per figure or table. `table1.csv` is the paper's Table 3 (the paper's Table 1 is the list of persons and Table 2 the parameters and their sources); `table1_paired.csv` holds the per-decision 95% intervals of its population cells and the paired differences against own judgement. Seeds: Fig. 1 rng(3); Fig. 2 seed 11; Fig. 3 seed 23; Table 3 seed 31; S-A 41, S-B 43, S-W 45, S-D 47, S-E 49, S-F 51, S-G/S-H/S-M/S-N 23, S-P 31, Table 4, S10 and S11 31, robustness 53. `table4_agent_gamma.csv` is the paper's Table 4 (the equal mix of the four persons after an agent hands over, by the agent's γ, with paired 95% half-widths against arrangement A); `sS10_handoff_persons.csv` gives every person, the follower and the mix (Table S10); `sS11_followers.csv` mixes a share f of followers with the four persons (Table S11); `convergence.csv` repeats the main cells under a finer evidence grid and more or fewer quadrature nodes (Supplementary Note 1). |
...
| After an overconfident agent (γ = 4) hands over, letting the person continue by their own policy (A) restores own judgement exactly for the calibrated and overconfident persons and gives the anxious and fearful persons the evidence the agent's longer search collected (group payoff 0.95 → 3.43 against own 3.26); two required checks before an irreversible action, with giving up open at any time (C), move only the acting-too-soon faults (overconfident 0.93 → 3.50, avoidable 4.5% → 1.2%; overconfident-and-fearful 1.71 → 3.62) at a cost of 0.05 to the calibrated person and leave missed opportunities where they were; protection both ways (B) moves only the giving-up faults (anxious 3.73 → 4.56, fearful 3.69 → 4.68) and lowers the mixed person's payoff; both together (D) give the group 4.32, avoidable 0.44%, missed 4.0% | `results/sS10_handoff_persons.csv`, Table S10, Supplementary Fig. S5 |
| Over the agent's γ in {1, 1.5, 2, 4, 6}, the equal mix of the four persons pays 4.68 / 4.50 / 4.26 / 3.43 / 3.37 when it may continue checking (A) and 4.31 / 4.40 / 4.34 / 4.09 / 3.98 when two checks are required before acting (C): required checks cost 0.37 under a calibrated agent, are neutral at γ = 1.5 to 2, and gain 0.65 at γ = 4 (paired 95% half-widths 0.02 to 0.03). A share f of followers, who take the agent's own decision, add no checks under A or B; with f = 0.86 and γ = 4 the gain from required checks rises to 2.30 ± 0.10 while the level under A falls to 1.28 | `results/table4_agent_gamma.csv`, `results/sS11_followers.csv`, Table 4, Table S11 |
